...
export default function Paramets({ setActiveTab = () => {}, activeTab = "" }) {
  const { task, showModal } = useSelector(({ validator }) => validator);
  const dispatch = useDispatch();

  // pull array or default
  const parametValues = Array.isArray(task?.paramet) ? task.paramet : [];
  ...
  const handleChange = (e) => {
    ...
    // clone and pad to match Echo.Paramets length
    const updated = [...parametValues];
    updated[index] = numericValue;

    // ensure full length
    const padded = Array(Echo.Paramets.length)
      .fill("")
      .map((_, i) => updated[i] ?? "");
    ...
    dispatch(SetPARAMS({ key: "paramet", value: padded }));
  };
    ...
    <MDBTable hover responsive className="mb-0">
      ...
      <tbody>
        {Echo.Paramets.map((field, index) => {
          const val = parametValues[index] ?? "";
          return (
            <tr key={`echo-paramet-${index}`}>
