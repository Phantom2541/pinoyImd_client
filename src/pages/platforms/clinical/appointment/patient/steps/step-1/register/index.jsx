...
const Register = ({ form, setForm }) => {
  ...
  const handleChange = (key, value) =>
    setForm({ ...form, user: { ...form?.user, [key]: value } });

  useEffect(() => {
    setForm((prev) => ({
      ...prev,
      user: { ...prev?.user, address: activePlatform?.branch?.address },
    }));
  ...
  return (
    <>
      <MDBBtn
        className="float-right px-2  ml-n5 py-1 zIndex-2 "
        size="sm"
        color="danger"
        rounded
      ...
      <div className="patient-personal-container mt-5">
          ...
          <div className="patient-form">
            ...
            <input
              ...
              onChange={({ target }) => {
                ...
                setForm({ ...form, user: { ...form.user, ...data } });
      ...
      <MDBInput
        ...
        value={email || generateEmail(form.user)}
      />
    </>
  );
};
