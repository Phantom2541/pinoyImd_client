...
const Holder = ({ refNo = {}, setRefNo = () => {} }) => {
  ...
  const { selected: onboarding } = useSelector(({ kiosk }) => kiosk);
  ...
  useEffect(() => {
    const { refNo: _refNo = {} } = onboarding;
    const { careOf: _careOf = {} } = _refNo;
    if (_careOf?.user?._id && _careOf?.category === holder) {
      setSelected(_careOf?.user);
      setRefNo({ ..._refNo, careOf: { ..._careOf, user: _careOf?.user?._id } });
    } else {
      setSelected({});
    }
  }, [onboarding, holder]);
