...
import { dateFormat, ENDPOINT, getAge } from "../../..";
...
// options list
const Patients = async ({ deals = [], options = {}, workbook }) => {
  ...
  const groupedArray = Object.entries(groupDeals).map(([date, deals]) => ({
  ...
  set.header({ worksheet, gross });
  set.main({
    worksheet,
    vouchers: groupedArray,
    isMembership: options?.isMembership,
  });
